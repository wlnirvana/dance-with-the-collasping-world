...
void dma_config(void)
{
    ...
    dma_init_struct.memory_addr  = (uint32_t)gif;
...
void LCD_ShowStringWithDelay(u16 x,u16 y,const u8 *p,u16 color, uint32_t delay)
{
	u16 x_offset = x;
	    while(*p!='\0')
    {       
        if(x>LCD_W-16){x=x_offset;y+=16;}
        if(y>LCD_H-16){y=x=0;LCD_Clear(RED);}
        LCD_ShowChar(x,y,*p,0,color);
        x+=8;
        p++;
...
void LCD_ShowGif(u16 x1, u16 y1, u16 x2, u16 y2) {
	LCD_Address_Set(x1,y1,x2,y2);
	for(int i = 0; i < 1600; i++) {
		for(int j = 7; j >= 0; j--) {
			u16 bit = (gif[gif_offset] >> j) & 1;
			if (bit & 1) {
				bit = 0;
			} else { 
				bit = ~0;
